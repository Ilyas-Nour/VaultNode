/**
 * 📄 PRIVAFLOW | Text to Word Converter
 * ---------------------------------------------------------
 * A document synthesis engine. Reads plain text streams
 * and constructs native OpenXML (.docx) bitstreams using docx.js.
 * 
 * Logic: Plain Text to Paragraph Mapping
 * Performance: O(n) Linear Synthesis
...
/**
 * 📄 TextToDocxTool Component
 * A high-security utility for local Text to Word reconstruction.
 */
const TextToDocxTool = memo(() => {
    // ✨ HOOKS & TRANSLATIONS
    const t = useTranslations("Tools.textToDocx");
    ...
    /**
     * ⚡ Synthesis Core
     * Reconstructs plain text as a native Word document.
     */
    const processFile = useCallback(async (file: File) => {
        ...
        try {
            ...
            // Preserve structure: split by newline and handle empty lines
            ...
            console.error("Text to Word error:", error);
            setErrorMsg(t('errorDesc') || "Failed to process text file.");
        } finally {
    ...
    const handleDownload = useCallback(() => {
        ...
        const newName = originalFile.name.replace(/\.txt$/i, ".docx");
...
TextToDocxTool.displayName = 'TextToDocxTool';

export default TextToDocxTool;
