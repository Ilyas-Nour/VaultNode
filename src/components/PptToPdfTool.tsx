/**
 * 📄 PRIVAFLOW | PowerPoint to PDF Converter
 * ---------------------------------------------------------
 * A structural document conversion engine. Unpacks PPTX
 * archives in-memory using JSZip, parses slide XML to extract
 * raw textual data, constructs an HTML representation, and 
 * renders a PDF locally using html2pdf.js.
 * 
 * Logic: JSZip -> XML parsing -> HTML synthesis -> html2pdf
...
import { Loader2, Projector, Upload, Download, CheckCircle2, Info, RefreshCw } from "lucide-react";
...
/**
 * 📄 PptToPdfTool Component
 * A high-security utility for local PowerPoint to PDF conversion.
 */
const PptToPdfTool = memo(() => {
    // ✨ HOOKS & TRANSLATIONS
    const t = useTranslations("Tools.pptToPdf");
    ...
    /**
     * ⚡ Extraction Core
     * Parses PPTX XML for text and renders to PDF blob
     */
    const processFile = useCallback(async (file: File) => {
        ...
        try {
            ...
            setErrorMsg(t('errorDesc') || "Failed to convert PPT file.");
        } finally {
    ...
    const handleDownload = useCallback(() => {
        ...
        const newName = originalFile.name.replace(/\.pptx?$/i, ".pdf");
                    ...
                    {!originalFile ? (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4 p-4 border border-zinc-800 bg-zinc-950">
                                <Projector className="w-8 h-8 text-zinc-600 shrink-0" />
...
PptToPdfTool.displayName = 'PptToPdfTool';

export default PptToPdfTool;
